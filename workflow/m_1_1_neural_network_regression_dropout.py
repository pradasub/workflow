...
def describe():
    ...
    return print(
        ...
        f'model = basic_model_classification(in_shape,total_hidden_layer,opt_func="Adam",\n'
        f'learning_rate=0.001,loss_func="mse",metric=["mse","mae"], dropout=0.2)\n'
        f'fit_model(X_train, y_train, model, batch_size=10, epochs=100, shuffle=True,\n'
        f'verbose=2,validation_split=0.1)\n'
    )

def basic_model_regression(
    in_shape,total_hidden_layer,opt_func='Adam',learning_rate=0.001,
    loss_func='mse',metric=['mse','mae'],dropout=0.2
):
