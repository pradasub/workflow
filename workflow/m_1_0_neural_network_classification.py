...
def describe():
    ...
    return print(
        f'Basic Neural Network Model\n'
        f'optimizers: SGD, RMSprop, Adagrad, Adadelta, Adam, Adamax,Nadam\n'
        f'model = basic_model_classification(\n'
        f'in_shape,out_shape,total_hidden_layer,opt_func="Adam",learning_rate=0.001,\n'
        f'loss_func="sparse_categorical_crossentropy",metric="accuracy")'
        f'fit_model(X_train, y_train, model, batch_size=10,\n'
        f'epochs=100, shuffle=True, verbose=2, validation_split=0.1)'
        f'y_pred_classes, y_pred_proba = model_pred(X_test,model, batch_size=10, verbose=0)'

    )

def basic_model_classification(
    in_shape,out_shape,total_hidden_layer,opt_func='Adam',
    learning_rate=0.001,loss_func='sparse_categorical_crossentropy',metric='accuracy'
):
