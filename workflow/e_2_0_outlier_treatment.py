...
def describe():
    return print(
        ...
        f'The module automatically finds numeric columns with specified number of unique_values\n'
        f'for outlier treatment\n'
        ...
        f'outlier_inter_quartile(data, num_unique=50)-> outlier based in inter quartile "qtr"\n'
        f'data = fill_outliers(\n'
        f'data,response,thres=2,num_unique=50,replace_median_or_mean="median", qtr_or_zscore="qtr")\n'
        f'-> data, response variable, thres= how many times interquartile range for example,\n' 
        f'num_unique = number of unique, values required for outlier treatment (default=50),\n'
        f'replace_median_or_mean="median", qtr_or_zscore="qtr" or "z_score"\n'
    )
...
def fill_outliers(
    data,response,thres=2,num_unique=50,replace_median_or_mean="median", qtr_or_zscore='qtr'
):
    reqd_columns = cols_for_outlier_treatment(data,num_unique)
